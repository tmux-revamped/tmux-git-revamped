...
# git_web_segment DIR -> provider PR, review, and issue counts, leading space.
git_web_segment() {
  local dir="${1}" url provider pr review issue
  url="$(_git_remote_url "${dir}")"
  provider="$(provider_from_url "${url}")"
  [[ -z "${provider}" ]] && return 0
  if [[ "${provider}" == "github" ]] && has_command gh; then
    pr="$(_gh_pr_count)"; review="$(_gh_review_count)"; issue="$(_gh_issue_count)"
  elif [[ "${provider}" == "gitlab" ]] && has_command glab; then
    pr="$(_glab_mr_count)"; review=0; issue=0
  else
    return 0
  fi
  [[ "${pr}" =~ ^[0-9]+$ ]] || pr=0
  [[ "${review}" =~ ^[0-9]+$ ]] || review=0
  [[ "${issue}" =~ ^[0-9]+$ ]] || issue=0
  echo " $(git_render_count pr "${pr}") $(git_render_count review "${review}") $(git_render_count issue "${issue}")"
}
